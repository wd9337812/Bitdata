...
本仓库 154 币中，2024 年以来新上市 105 个（2024: 24 / 2025: 79 / 2026: 2）：

- 上市后 72h 最大上行：中位数 **+33%**、均值 +55%、最大 **+418%**（CLOUSDT）；
- 上市后 72h 最大下行：中位数 **-22.5%**、均值 -25.9%、最深 -99.96%；
- 72h 绝对涨跌：中位数 21.5%、均值 28.1%。

结论：新币上市确实是单事件波动最大的山寨币类别（“赌博机会大”得到量化确认）。
监控器 new_listing 分支按每年约 40-80 个新币的速率积累样本，数周内即可形成
72h 波动的真实前向分布；能否交易仍需等样本判决（历史 72h 动量延续已被拒，
因为头部赢家集中）。
