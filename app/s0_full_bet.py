...
def s0_full_bet_profile_active(config: dict[str, Any]) -> bool:
    ...
    return bool(
        ...
        and version.startswith(("v4.4", "v4.5", "v4.6", "v4.7", "v4.8", "v4.9", "v4.10"))
        and stage == "S0"
    )
...
def is_s0_full_bet(candidate: dict[str, Any] | None, config: dict[str, Any]) -> bool:
    ...
    return bool(version.startswith(("v4.4", "v4.5", "v4.6", "v4.7", "v4.8", "v4.9", "v4.10")) and opportunity.get("full_bet_admitted"))
...
def build_s0_full_bet_sizing(
    *,
    ...
    version = str(config.get("opportunity_v4_strategy_version") or "").lower()
    if not version.startswith(("v4.5", "v4.6", "v4.7", "v4.8", "v4.9", "v4.10")) and int(consecutive_losses) >= int(config.get("opportunity_v44_loss_reduced_after", 2)):
