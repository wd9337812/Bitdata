...
DEFAULT_OUTPUT = (
    ROOT / "data" / "research" / "s0_30d_fast_exit_cross_year"
)
...
def frozen_profile() -> Profile:
    """Return the single profile registered before looking at its results."""
    return Profile(
        "momentum_30d_fast_exit_frozen_v1",
        ("ret_720h",),
        0.05,
        2.0,
        1.5,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Cross-year minute audit of a frozen faster 30-day momentum exit."
    )
...
def main() -> None:
    ...
    for delay in DELAYS_MINUTES:
        ...
        for cost in COSTS_PCT:
            ...
            funded.to_parquet(args.output / f"{name}.parquet", index=False)

    accepted = all(scenario_passes(item) for item in scenarios.values())
    result = {
        "experiment": "s0_30d_fast_exit_cross_year",
        "hypothesis": (
            "Keep the qualified 30-day cross-sectional selector, but use a "
            "faster public trend-style exit to recycle S0 capital more often."
        ),
        ...
        "scenarios": scenarios,
        "qualified_for_forward_shadow": accepted,
        "decision": (
            "historical_robustness_passed_requires_forward_shadow"
            if accepted
            else "reject_fast_exit_not_cross_year_robust"
        ),
        "warning": (
            "A strong recent year cannot override negative independent years "
            "or dependence on a few winning symbols."
        ),
    }
