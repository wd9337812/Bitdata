...
当前版本 `v0.23.0` / 策略 `V4.7.2`：依据 V4.6.2 以来的实盘与配对影子证据，回踩重新成为主要实盘形态，动量和预突破降为受限探索；同一行情事件按币种、方向、形态和市场状态去重。Setup 使用独立成本门槛，回踩最长持仓为 4 根 K 线，其他形态保持 2 根快速退出。V4.7.2 通过能力清单继承 V4.11 的全局市场、自适应、连续许可证、保护单与执行安全能力。MoE v1.2 已完成独立突破专家实验，但因未触碰测试集 PF 仅 `0.9791`，不替换线上 v1.1 影子模型。15% 压力风险上限、5U 硬停止、交易所保护单、流动性硬门和 API 优先级保持不变。
