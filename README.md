...
当前版本 `v0.29.0` / 策略 `V5.2`：S0 仍使用单仓全进全出短打，但证据改为按 30 分钟独立行情段统计，避免同一波行情重复刷高样本；选币优先横截面强度、结构、成本覆盖和可执行性，纯动量追涨只做影子。机会质量映射到 12%/18%/24%/30% 请求风险，实际下单还会按 5U 硬停止余量封顶。交易所止盈止损、5 秒持仓监督、流动性硬门、最小下单量和裸仓修复保持不变。旧版本证据仅用于历史复盘，不参与 V5.2 准入和仓位。MoE 继续只做研究影子，不接管实盘。完整说明见 `docs/v0.29.0-v52-evidence-edge-2026-07-29.md`。
