...
当前版本 `v0.9.4`：修复权益高点回撤后的恢复死锁。高点回撤仍会触发 `risk_off` 和强制冷却，但冷却结束后可通过当前 V4 `decision` 影子的连续正证据取得一次 `0.4x` 受限试单许可；只有 5U 硬停止会无条件撤销恢复资格。排序、杠杆、正常仓位和交易所保护单均未修改。详见 [v0.9.4 回撤恢复死锁修复留档](docs/v0.9.4-drawdown-recovery-deadlock-2026-07-17.md)。
