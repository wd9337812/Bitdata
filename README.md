...
当前版本 `v0.9.2`：V4 机会排序规则保持不变，本次只修复连续止损和运行期持仓保护。连续 4 笔亏损会独立触发新开仓保护，不再因当前版本累计收益仍为正而被忽略；持仓价格兜底、保护单检查和紧急退出固定使用关键 REST 预算。Binance 不支持原子替换已有 `closePosition` 止损时，系统保留已确认的旧止损，避免先撤单形成保护空窗。详见 [v0.9.2 V4 保护热修复留档](docs/v0.9.2-v4-protection-hotfix-2026-07-17.md)。
