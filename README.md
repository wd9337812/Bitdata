...
当前版本 `v0.7.0`：V3.2 继续作为 S0-S2 实盘基线，V3.3 作为同机会、同时间的配对影子候选运行。`risk_off`、恢复证据和首页状态只读取当前实盘版本；V3.1 与更早数据明确归档，不再混入最新策略结论。V3.3 满足样本、时长、PF 和市场状态要求后也只进入人工复核，不会自动替换实盘。5U 硬停止、交易所止盈止损保护和默认关闭的实盘换仓保持不变。详见 [v0.7.0 留档](docs/v0.7.0-v33-paired-shadow-2026-07-14.md)。
