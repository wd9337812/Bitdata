...
当前版本 `v0.13.0` / 策略 `V4.3.2`：核心机会不再依赖 A+/A/B 硬分层，而是按排名、扣费后保守期望、成本效率、中周期方向和流动性连续计算仓位置信度。合格核心机会的初始单笔风险在 `4%-7.5%` 间连续变化；受限探索不享受放大。持仓至少顺向运行 `0.55 ATR`，且交易所止损已收紧到保本并覆盖成本后，才允许同方向追加一次，初始与追加的名义风险合计永远不超过 `15%`。新版本前 24 小时最多试运行 5 个机会，净亏损 2 次即撤销许可证；版本权益从高点回撤 `8%` 时自动关闭连续放大和追加，退回基础受限仓位。5U 硬停止、交易所止盈止损、最小下单量、流动性门槛、API 频控和时间同步保持不变。详见 [V4.3.2 连续仓位与受保护追加留档](docs/v0.13.0-v432-continuous-risk-protected-addon-2026-07-19.md)。
