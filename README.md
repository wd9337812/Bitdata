...
当前版本 `v0.25.0` / 策略 `V4.7.4`：候选保护档案成为运行时唯一权威来源，持仓由独立的 5 秒监督循环检查，WebSocket 最新价和入场 ATR 优先，REST 只在数据缺失时兜底。S0 普通日亏损停牌默认关闭，达到 UTC 当日初始权益 `40%` 的已实现净利润且空仓后锁定当天新开仓；5U 硬停止、交易所端止盈止损、流动性硬门和裸仓修复保持不变。每次成功执行使用唯一 Binance 订单编号记录，避免把一次成交重复计为多条学习样本。MoE v1.1 已因线上负期望退役；v1.4 使用 119 个币、约 214 万条 Binance 候选训练，未触碰测试 PF `1.09`，但样本量、成本压力和跨市场状态不达标，因此不进入线上影子或实盘。
