...
当前版本 `v0.8.1`：V3.2 仍是 S0-S2 唯一实盘版本，V3.3 继续只做配对影子。恢复许可证最长保留 180 分钟，并在真正执行前检查当前实盘版本自身的币种/方向与信号类型证据；旧策略结果保持隔离，已被当前版本证明为负向的候选会被跳过，许可证继续等待其他机会。许可证只允许一笔受限仓位，开仓保护确认后立即消耗，亏损或保护失败会重新关闭准入。每日亏损基准按 UTC 日自动重置，策略版本高点独立于历史全局高点；5U 硬停止、交易所止盈止损保护和默认关闭的实盘换仓保持不变。详见 [v0.8.1 当前版本候选证据留档](docs/v0.8.1-release-evidence-hotfix-2026-07-15.md)。
