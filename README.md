...
当前应用版本 `v0.31.6`。S0 阶段的主路线是 **30 日山寨动量**：每天 UTC 窗口在至少 60 个高流动性永续币中，选择与 BTC 及山寨市场广度同向的最强/最弱端（多空都交易），按 20% 计划风险起步、2 倍杠杆、90% 保证金入场；权益达到 30U 后自动升到 22% 风险档。2.5 ATR 止损、3.5R 止盈、最长持有 5 天。只有当天没有可执行的山寨候选时才检查 BNB `28/56 日市场趋势共振` 后备。交易所止盈止损、5U 硬停止、0.5U 额外预留、裸仓修复和 API 限频保持不变。完整设计见 `docs/v0.31.0-s0-altcoin-30d-primary-2026-08-03.md`、`docs/v0.31.6-s0-altcoin-30d-short-gate-align-2026-08-04.md` 及后续升级留档。
...
- 信号：30 日截面强弱 + BTC 与山寨市场广度同向门，多空分别选择最强/最弱端，两个方向都交易
- 方向门：冻结审计全样本放行（多头 PF 2.52、空头 PF 1.49）；当前版本影子交易攒够 8 笔后滚动接管
- 单笔计划风险：`20%` 固定（22%/25% 阶梯未通过 bootstrap 稳健性审计：无硬停止概率 <90%，默认关闭；25%/30% 在 2026 年触发 5U 硬停止被拒）
