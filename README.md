...
当前版本 `v0.24.0` / 策略 `V4.7.3`：实盘、影子和决策记录使用同一行情事件编号完成可靠配对；回踩主路由增加扣费后净期望 `0.20%` 与保守净期望 `0.09%` 硬下界。S0 全仓短打在 3 分钟未兑现 `0.12%` 时主动退出，最长持仓 8 分钟，同时继续使用交易所端止盈止损。MoE v1.3 已使用 214 万条 Binance 历史样本和 5 分钟扣费后标签训练，但没有通过三折样本外门槛，因此不部署到实盘。V4.7.3 以最多 6 个独立机会的启动许可证受控验证；15% 压力风险上限、5U 硬停止、流动性硬门和 API 优先级保持不变。
