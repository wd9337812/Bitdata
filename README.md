...
当前版本 `v0.22.0` / 策略 `V4.11`：规则实盘策略保持不变。S0 MoE v1.1 将线上 `breakout` 明确映射到离线 `momentum` 专家，并按当前策略版本、当前模型版本分别展示线上门控影子与固定离线测试结果。线上数据只积累候选重训练证据，不会在 VPS 上自动改模型或接管实盘。15% 压力风险上限、5U 硬停止、交易所保护单、流动性硬门和 API 优先级保持不变。
