...
当前版本 `v0.11.0` / 策略 `V4.2`：在 V4 市场状态、方向和成本框架不变的前提下，将实盘准入拆成“核心通道 + 顺势受限探索通道”。受限探索只接收顺大盘方向的动量或确认回踩，默认要求本轮排名前 25%、质量分不低于 55、扣费后期望与成本确认，并以 `0.4x` 试仓；逆势和恐慌行情仍只做影子。V3 实盘、校准和配对实验已从当前执行链、API 和 Dashboard 归档，历史数据保留但不参与 V4.2 准入。5U 硬停止、交易所止盈止损、最小下单量、流动性、API 频控和时间同步均保持不变。详见 [V4.2 自适应双通道准入留档](docs/v0.11.0-v42-adaptive-admission-2026-07-17.md)。
