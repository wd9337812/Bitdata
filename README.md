...
当前版本 `v0.9.0`：新增独立 V4 候选引擎，按扣费后净期望、不确定性下界和同轮横截面排名选择机会。V4 分别记录“决策、探索、同机会对照”三类影子样本，避免只观察旧门槛通过者造成选择偏差；旧 V3.3 已归档，V3.2 暂时保持当前实盘基线，V4 默认只做影子实验，不会在证据不足时自动接管实盘。V4 不增加 Binance REST 扫描或下单调用，只复用现有候选和 WebSocket 行情；证据查询使用本地 SQLite 缓存。5U 硬停止、交易所止盈止损保护、API 限频和时间同步边界保持不变。详见 [v0.9.0 V4 候选引擎留档](docs/v0.9.0-v4-opportunity-engine-2026-07-16.md)。
