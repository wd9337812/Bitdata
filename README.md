...
当前版本 `v0.12.0` / 策略 `V4.3`：将影子样本从“每轮重复快照”改为“独立行情事件”，统一回踩信号别名，并按市场状态、方向、形态和入场阶段使用局部证据；大方向历史只调整仓位，不再直接否决新形态。实盘仍分核心与 `0.4x` 受限探索通道，小账户盘口门槛按实际订单动态计算，避免固定深度误杀。V4.2 回放中持续负期望的广泛下跌追空仍只做影子。5U 硬停止、交易所止盈止损、最小下单量、点差、API 频控和时间同步保持不变。详见 [V4.3 事件证据与动态准入留档](docs/v0.12.0-v43-event-evidence-routing-2026-07-18.md)。
