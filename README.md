...
当前版本 `v0.10.0` / 策略 `V4.1`：将选币改为“市场状态 × 方向 × 信号 × 入场阶段”的独立决策证据，加入动态成本下界、回踩优先和状态化保护；新策略在旧版本 `risk_off` 背景下可取得一张精确绑定版本、24 小时有效、默认最多 3 次、`0.4x` 的试运行许可证。许可证不绕过 5U 硬停止、交易所保护单、最小下单量、流动性和成本门槛。详见 [V4.1 状态自适应与新策略许可证留档](docs/v0.10.0-v41-state-canary-2026-07-17.md)。
