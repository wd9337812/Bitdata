...
当前版本 `v0.17.0` / 策略 `V4.7`：移除固定做空折扣，改为多空对称的顺势、中性、逆势校准。系统使用当前版本 12/24 小时实盘与事件级影子结果，在 `0.55x-1.15x` 范围内小步调整仓位倍率；样本充足后才允许小幅调整排名、净期望和确认门槛。V4.6.2 仅以 `0.25` 权重作为冷启动先验，不直接放行。校准读取本地数据库并缓存，不增加 Binance REST 请求。15% 压力风险上限、5U 硬停止、交易所保护单、流动性硬门和 API 优先级保持不变。
