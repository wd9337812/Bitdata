...
当前版本 `v0.4.1`：S0-S2 使用机会引擎 V3，通过全市场横截面、趋势结构、量价、衍生品、流动性和真实成本统一筛选 A+/A/B 机会。A+/A 实盘准入必须取得新鲜盘口并通过点差和深度检查，优先复用 WebSocket 快照，不增加常态 REST 压力；V3 信用与旧 V2 隔离，影子交易由 WebSocket 驱动。5U 硬停止与持仓止盈止损保护保持不变。详见 [机会引擎 V3 留档](docs/opportunity-engine-v3-2026-07-12.md)。
