...
当前版本 `v0.26.0` / 策略 `V5.0-S30`：S0 使用独立的单仓全进全出短打引擎，约 90% 可用保证金，普通压力风险从 12% 基线到最强机会 30% 上限，动态杠杆 3-10 倍，止损止盈和 5 秒持仓保护监督保持生效。三连净亏损只触发 20 分钟短冷却，随后自动回到 12% 风险基线；5U 硬停止、交易所保护单、流动性硬门、最小下单量和裸仓修复不变。旧 V3/V4 证据仅用于历史回放，不参与 V5 准入、信用或仓位。MoE 继续只做研究影子，不接管实盘开仓、仓位或退出。完整版本说明见 `docs/v0.26.0-v50-s30-direct-live-2026-07-28.md`。
