...
当前版本 `v0.9.1`：V4 已成为 S0-S2 当前实盘机会排序器。旧 V3.2 的 A+/A/B 分层、信用和历史亏损只保留诊断，不再直接放行或拦截实盘；V4 按扣费后净期望、同轮排名、流动性和版本隔离证据选择机会。证据尚少时，仅允许本轮前 15% 且模型扣费后期望不为负的趋势突破用 `0.5x` 受限探索；同类样本达到 40 笔、PF 与保守期望达标后才使用 `1.0x`。5U 硬停止、交易所止盈止损、最小下单量、API 限频和时间同步边界保持不变。详见 [v0.9.1 V4 实盘排序留档](docs/v0.9.1-v4-live-ranking-2026-07-16.md)。
