...
当前版本 `v0.8.0`：V3.2 仍是 S0-S2 唯一实盘版本，V3.3 继续只做配对影子。`risk_off` 恢复改为持久化许可证流程：当前版本影子数据达标并确认后，资格最多保留 180 分钟，等待后续真正满足质量、成本和交易所规则的候选，不再要求恢复证据与入场信号同一轮出现。许可证只允许一笔受限仓位，开仓保护确认后立即消耗，亏损或保护失败会重新关闭准入。每日亏损基准按 UTC 日自动重置，策略版本高点独立于历史全局高点；5U 硬停止、交易所止盈止损保护和默认关闭的实盘换仓保持不变。详见 [v0.8.0 恢复许可证留档](docs/v0.8.0-recovery-permit-2026-07-15.md)。
