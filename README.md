...
当前版本 `v0.12.2` / 策略 `V4.3.1`：在 V4.3 事件级证据基础上，将连续亏损约束从全局一刀切改为“局部组合熔断 + 软/硬两级保护”。同市场状态、方向、形态和入场阶段连续失效时只冻结该组合；试运行许可证亏损两次后进入观察，满足新影子证据后可按 `0.30x` 自动再签发。核心试运行也必须通过融合净期望和保守下界，`shadow_only` 只用于研究，不能决定实盘恢复。v0.12.2 为 Dashboard 与 runner 共用状态增加跨进程文件锁，避免许可证和局部熔断元数据在并发写入时互相覆盖。5U 硬停止、交易所止盈止损、最小下单量、点差、API 频控和时间同步保持不变。详见 [V4.3.1 局部熔断与许可证恢复留档](docs/v0.12.1-v431-local-circuit-recovery-2026-07-19.md)。
