...
当前版本 `v0.16.0` / 策略 `V4.6`：Dashboard 账户、持仓和保护状态改为私有 WebSocket 实时投影，断线时 REST 兜底；独立账户安全巡检每 2 秒检查变化，持仓时每 10 秒复核交易所止盈止损。后台全量扫描新增 90 秒看门狗，卡死后由容器自动恢复。选币方向新增 Binance 顶级交易者持仓/账户多空比、主动买卖量与未平仓量组成的“聪明钱”复合软评分，数据完整时最多修正 `±5` 分，不能单独放行或阻断开仓。V4.5 的连续准入、单仓全进全出、5U 硬停止、交易所保护单、流动性硬门和 API 优先级保持不变。详见 [V4.6 实时投影与聪明钱留档](docs/v0.16.0-v46-realtime-smart-flow-2026-07-21.md)。
