...
当前应用版本 `v0.30.0`，线上实盘策略仍为 `V5.2`。本版实现了 `V5.3` 行情融合候选：按市场状态路由顺势突破、预突破和回踩，使用横截面、中周期路径、入场质量、量能、资金流和流动性联合排序，并提供仅使用当前版本证据的全局小步校准。真实 Binance 历史样本外回放未达到自动晋级标准，因此 V5.3 不会自动替换 V5.2 实盘。交易所止盈止损、5U 硬停止、5 秒持仓监督、流动性硬门、最小下单量和裸仓修复保持不变。完整说明见 `docs/v0.30.0-v53-fusion-candidate-2026-07-29.md`。
