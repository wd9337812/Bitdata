...
当前版本 `v0.25.2` / 策略 `V4.7.4`：候选保护档案成为运行时唯一权威来源，持仓由独立的 5 秒监督循环检查，WebSocket 最新价和入场 ATR 优先，REST 只在数据缺失时兜底。S0 普通日亏损停牌默认关闭，达到 UTC 当日初始权益 `40%` 的已实现净利润且空仓后锁定当天新开仓；5U 硬停止、交易所端止盈止损、流动性硬门和裸仓修复保持不变。全 VPS 历史已按真实 Binance 1 分钟 K 线完成扣费回放；当前没有止盈止损候选同时通过验证段与最终测试段，因此实盘策略参数保持 V4.7.4。MoE v1.5 使用公开 Binance 数据低权重、VPS 影子中权重、可追溯实盘高权重在本地训练，最新独立测试仍为负期望，只在线上作为研究影子记录下一轮训练样本，不参与实盘准入、仓位或退出。
