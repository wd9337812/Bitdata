...
当前应用版本 `v0.30.1`，线上实盘策略仍为 `V5.2`。本版新增独立的 `28/56 日市场趋势共振` 研究影子：每天用高流动性永续币构造市场指数，只有快慢趋势同时向上时才建立 BTC 多头影子。该候选通过冻结样本外与执行延迟压力测试，但尚未获得实盘资格，不参与实盘准入、许可证、信用或仓位。交易所止盈止损、5U 硬停止、5 秒持仓监督、流动性硬门、最小下单量和裸仓修复保持不变。完整审计见 `docs/s0-market-tsmom-consensus-audit-2026-08-02.md`。
