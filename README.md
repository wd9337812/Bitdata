...
当前应用版本 `v0.30.2`，线上实盘策略仍为 `V5.2`。本版把 `28/56 日市场趋势共振 V2` 建成可审计的独立影子与待启用接管路径：每天用高流动性永续币构造市场指数，只有快慢趋势同时向上时才选择 BTC 多头，采用 ATR(10) 三倍移动止损、15% 灾难止损和 20 天持仓上限。它通过冻结样本外、滚动前推、参数邻域、延迟和成本压力测试，但默认不接管实盘；权益不足 10U 时也不会开仓。交易所止盈止损、5U 硬停止、裸仓修复和 API 限频保持不变。完整审计见 `docs/s0-market-tsmom-trailing-v2-audit-2026-08-03.md`。
