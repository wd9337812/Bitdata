...
当前版本 `v0.16.1` / 策略 `V4.6.1`：在 V4.6 的实时账户投影和聪明钱软评分之上，先完成 V4 基础预排，再只为决策短名单补齐聪明钱，最后重算最终排名；观察候选、未深查和中性回退状态在 Dashboard 明确区分。聪明钱数据完整时最多修正 `±5` 分，不能单独放行或阻断开仓。V4.5/V4.6 的连续准入、单仓全进全出、5U 硬停止、交易所保护单、流动性硬门和 API 优先级保持不变。详见 [V4.6.1 两阶段聪明钱排序留档](docs/v0.16.1-v461-two-stage-smart-flow-2026-07-21.md)。
