...
当前应用版本 `v0.31.4`。S0 阶段的主路线是 **30 日山寨动量**：每天 UTC 窗口在至少 60 个高流动性永续币中，选择与 BTC 及山寨市场广度同向的最强/最弱端，按 20% 计划风险起步、2 倍杠杆、90% 保证金入场；权益达到 30U 后自动升到 22% 风险档。2.5 ATR 止损、3.5R 止盈、最长持有 5 天。只有当天没有可执行的山寨候选时才检查 BNB `28/56 日市场趋势共振` 后备。交易所止盈止损、5U 硬停止、0.5U 额外预留、裸仓修复和 API 限频保持不变。完整设计见 `docs/v0.31.0-s0-altcoin-30d-primary-2026-08-03.md`、`docs/v0.31.2-s0-altcoin-30d-risk20-promotion-2026-08-04.md`、`docs/v0.31.3-s0-altcoin-30d-risk-tier-2026-08-04.md`、`docs/v0.31.4-s0-altcoin-30d-tp35-promotion-2026-08-04.md` 与 `docs/s0-market-tsmom-executable-v3-audit-2026-08-03.md`。
...
- 单笔计划风险：`20%` 起步，权益 ≥`30U` 自动升到 `22%`（20%/22% 历史按年重启均未触发 5U 硬停止；25%/30% 在 2026 年触发 5U 硬停止被拒）
- 杠杆：`2x`；保证金上限：`90%`；5U 硬停止并额外预留 `0.5U`
- 止损：`2.5 ATR`；止盈：`3.5R`；最长持有：`5 天`（退出档案审计显示 3.5R 全序列终值约为 2.5R 的 4 倍，按年重启均无硬停止）
- 候选入场窗口：UTC 评估后 `2 小时`，过期不追价
- 当日净利润锁：达到 `+200%` 后空仓锁定（历史回测未模拟当日锁仓，200% 让实盘与审计口径对齐，同时保留极端连赢的安全上限）

BNB 后备（仅在无新鲜山寨候选时）：

- `28/56 日市场趋势共振`，固定 `10.65%` 市场门、五天持有、20% 计划风险、2 倍杠杆
