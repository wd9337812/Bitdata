...
当前版本 `v0.16.2` / 策略 `V4.6.2`：根据 V4.4 以来的实盘与独立影子证据重排选币权重，重点提高方向资金流、量能持续、市场状态和中周期路径，降低已饱和的方向一致、横截面和流动性预测权重；动量与突破得到排序奖励，回踩与预突破降权。做空需要更强确认并降低计划风险，低收益持仓约 10 分钟后退出。聪明钱仍是独立 `±5` 分软修正，不能单独放行或阻断开仓。5U 硬停止、交易所保护单、流动性硬门和 API 优先级保持不变。
