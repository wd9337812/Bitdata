...
当前版本 `v0.21.0` / 策略 `V4.11`：线上规则策略改为更重视趋势延续、路径完整和防追价的非单调评分，极端放量、极端资金流不再自动加分。首版 Binance-only S0 MoE 使用严格时间切分训练并作为独立影子对照运行，不参与实盘准入、仓位或止盈止损。15% 压力风险上限、5U 硬停止、交易所保护单、流动性硬门和 API 优先级保持不变。
