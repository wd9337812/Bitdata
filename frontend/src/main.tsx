  ...
  return (
    <div className="panel signal-explain">
      ...
      <div className="explain-grid">
        ...
        <div><span>V4.11 准入通道</span><strong>{v4.admission_lane === "full_bet" ? "全仓短打" : "仅影子"}</strong></div>
        ...
        <div><span>V4.11 保守净期望</span><strong>{v4.enabled ? `${fmt(v4.lower_expected_net_pct, 3)}%` : "未形成 V4 触发"}</strong></div>
        <div><span>V4.11 成本比</span><strong>{v4.enabled ? `${fmt(v4.cost_ratio, 2)}x` : "未形成 V4 触发"}</strong></div>
        <div><span>V4.11 全局24小时</span><strong>{adaptive.ready ? (adaptive.adjustment_action || "已就绪") : "样本积累中"}</strong></div>
...
function V4OpportunityPanel({ funnel, performanceGuard }: { funnel: any; performanceGuard: any }) {
  ...
  return (
    <div className="panel">
      <div className="panel-head">
        <div>
          <h2>{v4.strategy_version || "v4.11"} 事件证据与全仓短打</h2>
      ...
      <div className="metrics">
        ...
        <MetricCard title="旧证据阻断" value={`${fmt(v4.local_circuit_blocked, 0)} 个`} sub="旧版本仅作历史背景，不参与 V4.11 准入、仓位或模型判断" tone="positive" />
    ...
    <section className="stack">
      <div className="panel">
        ...
        <div className="metrics">
          ...
          <MetricCard title="V4.11 运行方式" value="单仓全进全出" sub="健康延续优先，末端爆量降分；MoE仅作独立影子对照" />
        </div>
      </div>
      ...
      <section className="stack">
        <div className="panel">
          ...
          <div className="form-grid">
            {toggle("stage_routing_enabled", "按权益自动选择阶段", "推荐开启；当前 S0 使用 V4.11 单仓全进全出短打，后续阶段仍按权益自动切换")}
            ...
            {toggle("opportunity_v4_live_enabled", "V4.11 当前实盘排序", "当前版本证据独立，旧版本只供复盘，不参与准入和仓位")}
    ...
    <section className="stack">
      ...
      <details className="panel">
        ...
        <div className="form-grid">
          ...
          {text("opportunity_v4_strategy_version", "V4 当前版本号", "当前 v4.11；不同版本实盘和影子证据严格隔离")}
          {toggle("opportunity_v44_full_bet_enabled", "V4.11 S0 单仓全进全出", "只持有一个币种和一个方向；一次建仓、一次全平，禁止盈利追加和分批止盈")}
          {toggle("opportunity_v49_global_adaptive_enabled", "V4.11 全局动态校准", "只读取当前版本本地实盘与影子数据库，不增加 Binance API 请求，也不按单币种调参")}
          ...
          {toggle("opportunity_v410_global_regime_enabled", "V4.11 启用全局市场状态", "根据当前版本结果识别趋势或震荡；只影响全局方向倍率，不为单币种建永久参数")}
          {toggle("opportunity_v410_global_smart_flow_enabled", "V4.11 启用全局聪明钱代理", "把已有聪明钱复合信号汇总为全局背景，不新增 Binance 请求，也不单独触发开仓")}
