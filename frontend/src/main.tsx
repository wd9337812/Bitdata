...
function App() {
  ...
  const currentPerformanceScope = `${performanceGuard.active_strategy_family || "extreme_v4_roll"}@${performanceGuard.active_strategy_version || "v4.3.2"}`;
        ...
        {active === "overview" && (
          <section className="stack">
            <div className="metrics">
              ...
              <MetricCard
                title="软 / 硬保护边界"
                value={`${fmt(performanceGuard.soft_thresholds?.consecutive_losses, 0)} / ${fmt(performanceGuard.hard_thresholds?.consecutive_losses, 0)} 笔连亏`}
                sub={`软观察 ${fmt(performanceGuard.soft_thresholds?.observation_minutes, 0)} 分钟，硬冷却 ${fmt(performanceGuard.hard_thresholds?.cooldown_minutes, 0)} 分钟 · 回撤线 ${fmt(performanceGuard.soft_thresholds?.peak_drawdown_pct, 0)}% / ${fmt(performanceGuard.hard_thresholds?.peak_drawdown_pct, 0)}%`}
              ...
              <MetricCard
                ...
                sub="显示本轮仍被阻断的组合；只冻结同市场状态、方向、形态和入场阶段"
                tone={activeLocalCircuitBlocks > 0 ? "negative" : "positive"}
              />
              <MetricCard
                title="V4.3.2 版本回退"
                value={performanceGuard.release_equity_guard?.fallback_active ? "已回到基础仓位" : "连续仓位生效"}
                sub={`版本高点 ${fmt(performanceGuard.release_equity_guard?.peak_equity, 4)}U · 回撤 ${fmt(performanceGuard.release_equity_guard?.drawdown_pct, 2)}% / ${fmt(performanceGuard.release_equity_guard?.fallback_threshold_pct ?? 8, 2)}%`}
                tone={performanceGuard.release_equity_guard?.fallback_active ? "negative" : "positive"}
              />
              <MetricCard
                title="当日权益基准"
          ...
          <section className="stack">
            ...
            <div className="panel">
              <div className="panel-head">
                <div>
                  <h2>机会漏斗</h2>
                  <p>系统先大范围召回，再逐层粗排、精排和竞价；V4.3.2 统一信号口径，按市场结构、方向、事件级证据与成本计算扣费后净期望，只对精选币检查盘口。</p>
                </div>
              </div>
            ...
            <div className="panel">
              <div className="panel-head">
                <div>
                  <h2>候选币排名</h2>
                  <p>V4.3.2 是当前实盘排序器；核心机会用连续置信度计算仓位，受限探索仍保持小倍率，不再用 A+/A/B 标签硬切仓位。</p>
                </div>
              </div>
  ...
  return (
    <div className="panel signal-explain">
      ...
      <div className="explain-grid">
        ...
        <div><span>V4.3.2 准入通道</span><strong>{v4.admission_lane === "limited_exploration" ? "受限探索" : v4.admission_lane === "validated" ? "核心已验证" : v4.admission_lane === "core_provisional" ? "核心受限" : v4.admission_lane === "core_canary" ? "核心试运行" : "仅影子"}</strong></div>
        <div><span>连续仓位置信度</span><strong>{positionConfidence.applied ? `${fmt(Number(positionConfidence.confidence || 0) * 100, 1)}%` : "不适用"}</strong></div>
        <div><span>目标初始风险</span><strong>{positionConfidence.target_initial_risk_pct != null ? `${fmt(positionConfidence.target_initial_risk_pct, 2)}%` : "沿用通道基础风险"}</strong></div>
        <div><span>同仓追加资格</span><strong>{positionConfidence.add_on_eligible ? `符合；盈利 ${fmt(positionConfidence.add_on_trigger_atr, 2)} ATR 后再确认` : "不符合"}</strong></div>
        <div><span>追加后风险上限</span><strong>{positionConfidence.add_on_eligible ? `${fmt(positionConfidence.add_on_total_risk_cap_pct, 2)}%` : "-"}</strong></div>
        <div><span>V4.3.2 保守净期望</span><strong>{v4.enabled ? `${fmt(v4.lower_expected_net_pct, 3)}%` : "-"}</strong></div>
        <div><span>V4.3.2 成本比</span><strong>{v4.enabled ? `${fmt(v4.cost_ratio, 2)}x` : "-"}</strong></div>
        <div><span>动量确认</span><strong>{v4.enabled ? `${fmt(v4.momentum_confirmations, 0)} / ${fmt(v4.momentum_confirmations_required, 0)}` : "-"}</strong></div>
        <div><span>V4.3.2 同形态证据</span><strong>{v4.enabled ? `${fmt(v4.evidence?.selected?.trades, 0)} 笔 · ${pfLabel(v4.evidence?.selected?.profit_factor, v4.evidence?.selected?.trades)}` : "-"}</strong></div>
        <div><span>局部熔断</span><strong>{localCircuit.blocked ? "已冻结该组合" : localCircuit.restored ? "影子验证已恢复" : "未触发"}</strong></div>
...
function FunnelPanel({ funnel }: { funnel: any }) {
  ...
  return (
    <div className="metrics">
      ...
      <MetricCard
        title={funnel?.opportunity_v4?.enabled ? "V4.3.2 实盘候选" : "剥头皮信号"}
...
function V4OpportunityPanel({ funnel }: { funnel: any }) {
  ...
  return (
    <div className="panel">
      <div className="panel-head">
        <div>
          <h2>{v4.strategy_version || "v4.3.2"} 事件证据与连续质量仓位</h2>
          <p>先判断方向和入场结构，再按排名、保守净期望、成本、流动性与局部证据计算连续置信度；标签只用于解释，不再用 A+/A/B 硬切仓位。</p>
        </div>
      </div>
      <div className="metrics">
        ...
        <MetricCard title="结构可交易" value={`${fmt(v4.structure_ready, 0)} 个`} sub="顺大盘或中周期方向，且入场形态有效" />
        <MetricCard title="盘口可执行" value={`${fmt(v4.liquidity_ready, 0)} 个`} sub="按本次预计下单额动态检查深度与点差" />
        <MetricCard title="局部熔断" value={`${fmt(v4.local_circuit_blocked, 0)} 个`} sub="只冻结失效组合，不影响其他市场状态与形态" tone={Number(v4.local_circuit_blocked || 0) > 0 ? "negative" : "positive"} />
        <MetricCard title="核心试运行" value={`${fmt(v4.canary_ready, 0)} 个`} sub="精确绑定 V4.3.2 许可证，不绕过硬风控" tone={Number(v4.canary_ready || 0) > 0 ? "positive" : ""} />
        ...
        <MetricCard title="连续初始风险" value="4% - 7.5%" sub="仅核心通道生效，仍叠加方向证据和全局保护" />
        <MetricCard title="保护后追加" value="最多 1 次" sub="先盈利 0.55 ATR 并确认保本止损，总风险最多 15%" />
      </div>
    ...
    <section className="stack">
      <div className="panel">
        <div className="panel-head">
          <div>
            <h2>策略版本一眼看懂</h2>
            <p>{active.strategy_version || "V4.3.2"} 是当前实盘版本。每笔影子都会记录核心、受限探索或仅影子通道，版本间证据严格隔离。</p>
          </div>
        ...
        <div className="metrics">
          ...
          <MetricCard title="V4.3.2 运行方式" value="连续质量仓位 + 受限探索" sub="核心初始风险按连续置信度落在 4%-7.5%；受限探索不放大；盈利并先锁保本后才可追加一次" />
        </div>
      </div>
      ...
      <div className="panel table-wrap">
        ...
        <table><thead><tr><th>类型</th><th>含义</th><th>已结束</th><th>胜率</th><th>PF</th><th>净收益</th><th>成本</th></tr></thead><tbody>
          {evidenceTypes.map((row: any) => <tr key={row.evidence_type}><td>{row.evidence_type === "decision" ? "决策样本" : row.evidence_type === "exploration" ? "探索样本" : "同机会对照"}</td><td>{row.evidence_type === "decision" ? "V4.3.2 排名靠前且按事件去重的机会，用于当前版本证据" : row.evidence_type === "exploration" ? "从仅影子机会中抽样，用于检查是否漏判" : "同一时刻用简单规则做基线，不参与准入"}</td><td>{fmt(row.opportunities || row.closed, 0)}</td><td>{fmt(row.win_rate, 1)}%</td><td>{pfLabel(row.profit_factor, row.closed)}</td><td>{fmt(row.net_pnl, 4)} U</td><td>{fmt(row.cost, 4)} U</td></tr>)}
      ...
      <div className="panel table-wrap">
        ...
        <table><thead><tr><th>通道</th><th>独立机会</th><th>已结束</th><th>胜率</th><th>PF</th><th>净收益</th><th>成本</th></tr></thead><tbody>
          ...
          {!admissionLanes.length && <tr><td colSpan={7}>V4.3.2 刚启用，等待第一批按通道分类的事件级影子结果。</td></tr>}
        </tbody></table>
      </div>
      ...
      <section className="stack">
        <div className="panel">
          ...
          <div className="form-grid">
            {toggle("stage_routing_enabled", "按权益自动选择阶段", "推荐开启；当前 S0-S2 使用 V4.3.2 事件证据与连续质量仓位滚仓")}
            ...
            {toggle("opportunity_v4_live_enabled", "V4.3.2 当前实盘排序", "推荐开启；旧策略实验已归档")}
            {toggle("opportunity_v432_continuous_sizing_enabled", "核心机会连续仓位", "推荐开启；不是 A+/A/B 硬分层，初始风险按连续置信度计算")}
            {number("opportunity_v432_initial_min_risk_pct", "核心初始风险下限%", "默认 4%")}
            {number("opportunity_v432_initial_max_risk_pct", "核心初始风险上限%", "默认 7.5%，仍受许可证与全局保护")}
            {toggle("opportunity_v432_add_on_enabled", "盈利锁保本后追加一次", "仅强核心机会；确认交易所保本止损后才可执行")}
            {number("opportunity_v432_add_on_total_risk_cap_pct", "初始加追加总风险上限%", "默认 15%，不会每单固定用满")}
            {number("opportunity_v432_release_fallback_drawdown_pct", "版本自动回退回撤%", "默认 8%；触发后撤销新增激进能力")}
    ...
    <section className="stack">
      ...
      <details className="panel">
        ...
        <div className="form-grid">
          ...
          {text("opportunity_v4_strategy_version", "V4 当前版本号", "当前 v4.3.2；改变模型后必须换版本，避免混用历史证据")}
          ...
          {number("opportunity_v4_bootstrap_min_model_expectancy_pct", "V4.3.2 核心试运行最低融合净期望%", "默认 0.10%；许可证也必须通过融合期望与保守下界")}
          {number("opportunity_v4_bootstrap_risk_multiplier", "V4.3.2 核心基础通道倍率", "默认 0.4；连续质量仓位会在核心准入后接管初始风险")}
          {toggle("opportunity_v43_exploration_enabled", "V4.3.2 顺势受限探索", "只对顺势动量、确认回踩或确认突破生效；不会获得连续仓位加码")}
          ...
          {toggle("opportunity_v432_continuous_sizing_enabled", "启用连续质量仓位", "核心机会按排名、保守净期望、成本、方向和流动性连续计算，不使用 A+/A/B 硬分层")}
          {number("opportunity_v432_initial_min_risk_pct", "核心初始风险下限%", "默认 4%；已有负向方向证据仍可继续折扣")}
          {number("opportunity_v432_initial_max_risk_pct", "核心初始风险上限%", "默认 7.5%；仍受阶段、全局保护和许可证限制")}
          {toggle("opportunity_v432_add_on_enabled", "盈利后允许一次受保护追加", "必须先上涨或下跌 0.55 ATR，并确认交易所保本止损后才会执行")}
          {number("opportunity_v432_add_on_min_confidence", "追加最低连续置信度", "默认 0.75；受限探索不会获得追加资格")}
          {number("opportunity_v432_add_on_min_cost_ratio", "追加最低收益成本比", "默认 8 倍；防止手续费和滑点吞掉追加收益")}
          {number("opportunity_v432_add_on_total_risk_cap_pct", "初始加追加总风险上限%", "默认 15%；这是绝对上限，不是每次都用满")}
          {number("opportunity_v432_release_fallback_drawdown_pct", "当前版本自动回退回撤%", "默认 8%；触发后撤销连续仓位和追加，回到基础受限仓位")}
          ...
          {number("opportunity_v4_admission_min_trades", "V4.3.2 核心受限准入样本", "默认 200 个事件级决策机会；按市场状态、方向、信号和入场阶段分组")}
          {number("opportunity_v4_admission_min_profit_factor", "V4 同类最低 PF", "默认 1.10，且必须是扣费后结果")}
          {number("opportunity_v4_admission_min_lower_expectancy_pct", "V4.3.2 核心保守净期望下限%", "默认 0%；考虑样本不确定性后的下界必须大于零")}
          ...
          {number("strategy_canary_max_losses", "试运行最大亏损笔数", "默认 2 笔；达到后进入观察，不是永久停用")}
          ...
          {number("strategy_canary_reissue_multiplier", "再签发试单倍率", "默认 0.30x")}
          {number("strategy_canary_reissue_max_opportunities", "再签发最多机会", "默认 2 次")}
